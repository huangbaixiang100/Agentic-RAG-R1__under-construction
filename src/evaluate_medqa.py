...
def evaluate_model_on_medqa(
    ...
    Args:
        ...
        start_sample: 起始样本索引，用于指定从哪个样本开始评估
    """
    ...
    # 根据start_sample和num_samples选择样本范围
    if start_sample > 0 or (num_samples is not None and num_samples < len(dataset)):
        end_sample = start_sample + num_samples if num_samples is not None else len(dataset)
        end_sample = min(end_sample, len(dataset))  # 确保不超过数据集大小
        if start_sample >= len(dataset):
            raise ValueError(f"起始样本索引 {start_sample} 超过数据集大小 {len(dataset)}")
        dataset = dataset.select(range(start_sample, end_sample))
        print(f"选择样本范围: {start_sample} 到 {end_sample-1} (共 {end_sample-start_sample} 个样本)")
...
def main():
    ...
    parser.add_argument("--start_sample", type=int, default=0, help="起始样本索引（从0开始计数）")
