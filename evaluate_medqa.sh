...
# MedQA数据集评估脚本 - 复制Qwen2.5-1.5B配置并运行评估
# 使用方法: ./evaluate_medqa.sh

set -e  # 遇到错误时退出

# 配置参数
#MODEL_PATH="/data/xiaobei/dhx/LLaMA-Factory-main-new/models/promed-qwen2.5-1.5b-sft-3epoch-merged"
MODEL_PATH="/home/xiaobei/hbx/HHHAgentic-RAG-R1__under-construction/checkpoints/promed-qwen25-1.5b-correctness-reward-tokenlevel-noshapley/2025-07-16/step-0200"
#MODEL_PATH="/home/xiaobei/hbx/HHHAgentic-RAG-R1__under-construction/checkpoints/promed-qwen25-1.5b-correctness-reward714-batch4/2025-07-14/step-0100"
...
EVALUATE_SCRIPT="${SCRIPT_DIR}/src/evaluate_medqa.py"
MEDQA_DATA_FILE="/home/xiaobei/hbx/ood补充实验/medqa_test_convo.jsonl"

echo "=========================================="
echo "MedQA数据集评估脚本开始执行"
echo "模型路径: ${MODEL_PATH}"
echo "数据文件: ${MEDQA_DATA_FILE}"
echo "=========================================="
...
# 检查MedQA数据文件是否存在
if [ ! -f "${MEDQA_DATA_FILE}" ]; then
    echo "错误: MedQA数据文件不存在: ${MEDQA_DATA_FILE}"
    exit 1
fi
...
echo "步骤3: 开始MedQA数据集评估..."

# 获取MedQA数据集的总样本数
TOTAL_SAMPLES=$(wc -l < "${MEDQA_DATA_FILE}")
echo "MedQA数据集总样本数: ${TOTAL_SAMPLES}"
...
echo "运行命令: python ${EVALUATE_SCRIPT} --model_path ${MODEL_PATH} --local_model --dataset_split test --num_samples ${TOTAL_SAMPLES} --batch_size 10 --max_new_tokens 2048 --max_generate_iterations 10 --temperature 0.3 --output_dir evaluation_results --test_file ${MEDQA_DATA_FILE}"
...
python3 "${EVALUATE_SCRIPT}" \
    ...
    --test_file "${MEDQA_DATA_FILE}" \
    --log_file evaluation_medqa_$(date +%Y%m%d_%H%M%S).log
